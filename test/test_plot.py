...
class TestPlot(unittest.TestCase):
    ...
    #     main(
    #         config='plot_test_files/config_plot_line_dot_freq.json',
    ...
    def test_example_preference(self):
        ...
        map_paths = [
            "results/preference/K1_0.pdf",
            "results/preference/K3_0.pdf",
        ]
        for map_path in map_paths:
            assert os.path.exists(map_path)
            assert os.path.getsize(map_path) > 0
    ...
    def test_example_dic(self):
        ...
        map_paths = [
            "results/DIC/K1_0.pdf",
            "results/DIC/K3_0.pdf",
        ]
        for map_path in map_paths:
            assert os.path.exists(map_path)
            assert os.path.getsize(map_path) > 0
    ...
    #     )
    #     map_paths = [
    #         "results/pie/K1_0.pdf",
    #         "results/pie/K3_0.pdf",
    #     ]
    #     for map_path in map_paths:
    #         assert os.path.exists(map_path)
    #         assert os.path.getsize(map_path) > 0
    ...
    # def test_example_idw_map(self):
    #     main(
    #         config='plot_test_files/config_plot.json',
    #         plot_types=[PlotType.map]
    #     ,
    #     )
    #     map_paths = [
    ...
    # def test_get_cluster_colors_five(self):
    #     custom_colors =  ["#1b9e77","#d95f02","#7570b3","#e7298a","#66a61e"]
    #     test_color = np.array([colors.to_rgba(c) for c in custom_colors])
    #
    #     plot = Plot()
    #     color = plot.get_cluster_colors(n_clusters=5,custom_colors=custom_colors)
    #     self.assertTrue((color==test_color).all())
    #     #self.assertTrue(color==test_color)
    #
